...
/// <summary>Keys and values of one attention layer for every position decoded so far: [batch·heads, capacity, headDim].</summary>
public sealed class KeyValueCache : IDisposable
{
    internal KeyValueCache(int rows, int capacity, int headDim, Device device)
    {
        // Created outside any TensorScope: caches are usually created lazily inside a scoped prefill step.
        Keys = Tensor.Empty([rows, capacity, headDim], device, zeroed: true, track: false);
        Values = Tensor.Empty([rows, capacity, headDim], device, zeroed: true, track: false);
    }

    /// <summary>Cached keys, [batch·heads, capacity, headDim].</summary>
    public Tensor Keys { get; }

    /// <summary>Cached values, [batch·heads, capacity, headDim].</summary>
...
public sealed class DecodingContext : IDisposable
{
    ...
    public DecodingContext(Device device, int batch, int capacity)
    {
    ...
    internal KeyValueCache CacheFor(object owner, int rows, int headDim)
    {
        ...
        {
            _caches[owner] = cache = new KeyValueCache(rows, Capacity, headDim, Device);
        }
