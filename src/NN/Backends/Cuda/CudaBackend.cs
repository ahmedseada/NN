...
using System.Text;
using static NN.Backends.Cuda.CudaDriver;
...
internal sealed unsafe class CudaBackend : Backend
{
    ...
    private readonly IntPtr _sigmoid, _tanh, _relu, _square;
    private readonly IntPtr _sigmoidBwd, _tanhBwd, _reluBwd, _squareBwd;
    ...
    {
        if (Environment.GetEnvironmentVariable("NN_DISABLE_CUDA") is "1" or "true")
        {
            return (0, "disabled by the NN_DISABLE_CUDA environment variable");
        }
    ...
    private static IntPtr LoadModule(string ptx)
    {
        ...
        fixed (byte* p = image)
        {
            ...
            if (result != 0)
            {
                ...
                throw new CudaException($"The CUDA driver could not JIT-compile the NN kernels (error {result}). {details}");
            }
    ...
    {
        nuint bytes = (nuint)Math.Max(length, 1) * sizeof(float);
