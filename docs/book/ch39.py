"""Chapter 39 — Serving Models over HTTP."""
from gen import *
...
def build():
    return page(
        ...
        h2("39.1 A prediction API in one file"),
        deriv("Creating it", [
        ...
        para("The predictions match the console sample's (<code>$390,612</code> and <code>$192,873</code>, " + ch("regression") + "). "
             ...
             "concurrent requests need no lock (" + ch("inference") + ", Section 38.4)."),
        ...
        reftable(["Endpoint", "Returns"], [
            ...
            ["<code>POST /api/chat</code>, <code>GET /api/tags</code>, <code>/api/ps</code>, <code>/api/version</code>", "Ollama-compatible chat with streaming, thinking, tools, options and keep_alive (" + ch("textgen") + ")"],
        ...
        ], caption="Table 39.1 — NeuralSharp.Samples.GptApi"),
        snippet("""
            var builder = WebApplication.CreateBuilder(args);
        ...
        ], caption="Table 39.2 — Serving in production"),
        practice([
