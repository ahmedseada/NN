...
if (SampleOptions.Parse(args,
        ...
        ("length", "characters to generate (default 400)")) is not { } options)
{
    return 0;
}
...
using (gpt)
{
    ...
    foreach (var (prompt, i) in prompts.Select((p, i) => (p, i)))
    {
        ...
        Console.WriteLine($"Prompt \"{prompt}\" (temperature {settings.Temperature}):");
        Console.WriteLine("  " + (prompt + result.Text).Replace("\n", "\n  "));
        Console.WriteLine($"  -> {m.GeneratedTokens} characters in {m.TotalMs:F0} ms ({m.TokensPerSecond:F0}/s), " +
                          $"average confidence {m.AverageProbability:P0}, perplexity {m.Perplexity:F2}");
        if (gpt.Config.Corpus == "built-in grammar")
        {
            var (valid, total) = Grammar.Score(result.Text);
